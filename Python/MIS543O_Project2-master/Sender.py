...
class Sender(BasicSender.BasicSender):
    ...
    def start(self):
        ...
        while not msg_type == 'end':
            ...
            while response == None or Checksum.validate_checksum(response.decode()) is False:         
                self.send(packet.encode())
                response = self.receive(.5)

            ack_type, ackno, data, checksum = self.split_packet(response.decode())

            while ackno != str(seqno +1):
                self.send(packet.encode())
                response = self.receive(.5)
